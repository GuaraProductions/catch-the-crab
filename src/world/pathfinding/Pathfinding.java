	...
	public static ArrayList<Node> findPath(World world, int xstart,int ystart,int xfinal, int yfinal) {
		...
		while(openList.size() > 0 ) {
			...
			for ( int i = 0; i < 9 ; i++) {
				...
				Block tile = World.getBlock(x+x1,y+y1);
				
				if (tile == null) continue;
				if (tile instanceof SolidBlock) continue;
				if (i == 0) {
					
					Block aux = World.getBlock(x+x1+1,y+y1);
					Block aux2 = World.getBlock(x+x1,y+y1+1);
					if( aux instanceof SolidBlock || aux2 instanceof SolidBlock) {
					...
					Block aux = World.getBlock(x+x1,y+y1-1);
					Block aux2 = World.getBlock(x+x1+1,y+y1);
					if( aux instanceof SolidBlock || aux2 instanceof SolidBlock) {
